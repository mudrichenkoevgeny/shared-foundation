...
/**
 * DTO representing user session metadata **with masked IP address** when policy restricts sensitive fields.
 *
 * Used in management flows where the caller has a **masked** session-list permission (see
 * [SessionPermissionCodes]); not used for the account owner’s own open-API session list when that list
 * is documented as unmasked ([UserSessionUnmaskedResponse]).
 *
 * @property id The unique identifier of the session.
 * @property userId The ID of the user who owns this session.
 * @property maskedIdentifier The masked login value for the credential used to create this session (e.g. masked email or phone).
 ...
 * @property maskedIpAddress The masked IP address from which the session was initiated or last accessed.
 ...
 */
@Serializable
data class UserSessionMaskedResponse(
    ...
    @SerialName(CommonApiFields.MASKED_IP_ADDRESS)
    val maskedIpAddress: String? = null,
